...
export type KernelEvent = 
  ...
  | { type: 'TICK'; dt: number; timestamp: number } 
  | { type: 'OBSERVATION_INGEST'; observation: Observation; timestamp: number } // Sensor Driver Input
  ...
  | { type: 'SAFETY_INTERDICTION'; riskLevel: number; action: string; timestamp: number } // The Guard blocked something
  | { type: 'ADAPTATION'; parameter: string; oldValue: number; newValue: number; timestamp: number };
...
export type UserSettings = {
  ...
  // The Safety Registry is now part of the Kernel data, but persisted here
