...
export function Footer({ selectedPatternId, setSelectedPatternId }: FooterProps) {
  ...
  const isPatternLocked = (pattern: BreathPattern): { locked: boolean; reason?: string } => {
    ...
    if (pattern.tier === 3) {
      ...
      const perfectSessions = Object.values(userSettings.safetyRegistry).reduce((acc: number, curr: unknown) => {
          // Explicit cast for curr inside reduce callback
          const profile = curr as SafetyProfile;
